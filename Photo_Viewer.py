...
def set_as_default_viewer(extensions=None, lang="tr"):
    tt = TRANSLATIONS.get(lang, TRANSLATIONS["en"])
    if not HAS_WINREG:
        return False, tt.get("err_no_winreg", TRANSLATIONS["en"]["err_no_winreg"])
    if sys.platform != "win32":
        return False, tt.get("err_windows_only", TRANSLATIONS["en"]["err_windows_only"])
    ...
    try:
        ...
        with winreg.CreateKey(winreg.HKEY_CURRENT_USER, f"Software\Classes\{PROG_ID}") as key:
            winreg.SetValueEx(key,"",0,winreg.REG_SZ,tt.get("default_file_desc", TRANSLATIONS["en"]["default_file_desc"]))
...
class Win7Viewer(QMainWindow):
    ...
    def action_set_default(self):
        ok,msg=set_as_default_viewer(lang=self.current_lang)
        if ok:
